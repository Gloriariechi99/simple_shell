...
{
	char *buf, **cmd, *cmd_path, *pdr = NULL;
	size_t size = 0;
	static int ret, ct;
	ssize_t wads = 0;
	Alias *_alias;
	...
	if (isatty(STDIN_FILENO) && argc == 1)/*not piped*/
		ret = loop_shell(argv[0], &cmd_path, &_alias, pdr, &ct);
	else if (isatty(STDIN_FILENO) == 0) /*piped*/
	{
		...
		if (cmd != NULL)
		{
			if (has_separator(&cmd)) /* commands are separated by ||, && or ; */
				...
				in_builts(&cmd, &ct, &pdr, argv[0], &_alias, &cmd_path);
		}
		free_1(&buf);
	}
...
void package(char ***cmd, char **cmd_path, char *argv)
{
	...
	while (cmd[0][i] != NULL)
	{
		if (i == 0)/* exec name*/
		{
			...
			{
				pkg[i] = (char *) malloc(sizeof(char) * (_strlen(cmd_path[0]) + 1));
				if (pkg[i] == NULL)
					exerror("Malloc Error");
...
void option_folder_file_etc(char ***cmd, char **cmd_path, char *argv)
{
	...
	if (stat_val == 0 && (mode.st_mode & S_IFMT) == S_IFDIR)
	{
		package(cmd, cmd_path, argv);
		return;
	}
