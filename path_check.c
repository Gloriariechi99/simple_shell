...
void command_file(char **cmd_path, char **argv)
{
	...
	if (fd == -1)
	{
		path_error(argv[0], argv[1], "non", "open");
		return;
	}
	...
	else
	{
		...
		if (resize > 0)/**resizing occured coz value set 8 line above and must have entered loop above to change**/
		{
			lseek(fd, 0, SEEK_SET);/*goto begining of file*/
			buf[wads - 1] = '\0'; /**terminate read string for realloc because it is terminated with \n which confuses string functions**/
		...
		close(fd);
		buf[wads - 1] = '\0'; /**terminate read string because it is terminated with \n  which confuses string functions including interpret**/
		cmd = interpret(buf);
		free_1(&buf);
