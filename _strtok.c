...
void show_alice(Alias **Alice, int *found, const char *cmp)
{
	...
	int i, j = *found; /*j used to counter *found++ giving error, value computed not used*/
 ...
 * @joint: container for alias
 * @x: alias_names index
 * @alias_names: list of aliases
 *
 * Return: nothing
 */
void edit_elias(Alias **Alice, int i, int *found, char **joint, int x, char **alias_names)
{
	char strtak[50], *com;
	int j = *found; /*j used to counter *found++ giving error, value computed not used*/

	if (_strcmp(Alice[0][i].name, alias_names[x]) == 0)/*alias exists, modify the value*/
	{
